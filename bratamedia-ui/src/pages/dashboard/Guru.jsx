...
const Guru = () => {
  ...
  const getDataAllGuru = async () => {
    setLoading(true);
    try {
      const res = await axios.get(
        `${
          import.meta.env.VITE_API_ENDPOINT
        }/guru?page${page}&search=${search}`,
        {
  ...
  // Delete Action
  const deleteGuruHandler = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_ENDPOINT}/guru/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } catch {
  ...
  return (
    <div>
      <h2 className='text-2xl font-bold mb-4'>Data Guru </h2>
      ...
      <div className='mb-4 flex justify-between items-center'>
        ...
        <button
          ...
          onClick={() => {
            navigate("/dashboard/guru/tambah");
          }}>
          Tambah Data Guru
        </button>
      </div>
                ...
                <tr key={i} className={"py-2 border-b"}>
                  ...
                  <td className='p-3'>
                    <button
                      ...
                      onClick={() => {
                        navigate(`/dashboard/guru/edit/${d.id}`);
                      }}>
                      Update
                    </button>
                    <button
                      className='bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600'
                      onClick={() => deleteGuruHandler(d.id)}>
                      Delete
                    </button>
                  </td>
                </tr>
