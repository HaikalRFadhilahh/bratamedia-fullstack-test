...
const UpdateSiswa = () => {
  ...
  // Get Old Value Siswa
  const getDefaultSiswa = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_ENDPOINT}/siswa/${id}`,
        {
  ...
  return (
    <div className='p-6 bg-white shadow-md rounded-lg'>
      <h2 className='text-2xl font-bold mb-6'>Update Data Siswa</h2>
      ...
      <div className=' p-6 rounded-lg'>
        <form onSubmit={handleSubmit}>
          ...
          <select
            ...
            {listKelas.map((d, i) => {
              return (
                <option value={d.id} key={i} selected={d.id == request.kelasId}>
                  {d.nama}
                </option>
              );
            })}
